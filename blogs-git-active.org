...
** COMMENT WAITTING [#D] Theme:搭建git服务器							 :@Blog:@Git:
...
2. 基于Gitlite搭建git服务器
   ...
   4. Gitolite管理员的那些事: (本地完成)
	  1. 配置文件管理
		 ...
		 5. 配置文件模板
			1. gitolite-admin/conf/gitolite.conf
			   3. # 管理员组
			   4. @administrators = admin  ytulinjiajun
			   5. 
			   6. #全局配置
			   7. repo @all		           # 针对所有的仓库配置
			   8. ----R		= @administrators 
			   9. 
			   10. # 共有通配符仓库,合作项目
			   11. repo public/.+$      
			   ...
			   13. ----
		 	   14. 
			   15. # 个人通配符仓库,私人项目
			   16. repo private/ytulinjiajun/.+$
			   17. ----C       = @administrators
			   18. ----
			   19. 
			   20. include "private-repo.conf"
			   21. include "public-repo.conf"
	  2. 权限控制
		 1. 管理员组:
	  ...
	  6. 创建新版本库
		 1. 在服务器直接创建
			1. 由管理员在/conf/gitolite.conf中加入一段新的版本库的授权指令
               #+BEGIN_SRC 
               repo new-repo
                    规则
               #+END_SRC
			2. 对本地的gitolite-admin.git执行push操作
			3. 在服务器上便创建了一个名为new-repo的版本库
			4. 将new-repo版本库clone下来进行开发,然后推送
		 2. 在客户端创建
			1. 由管理员修改服务器上git账号家目录下面的.gitolite.rc文件以支持通配符版本库,添加配置: $GL_WILDREPOS = 1;
			2. 使用通配符版创建版本库:
			   #+BEGIN_SRC 
			   repo ytulinjiajun/.+$
			   #+END_SRC
...
| user  | access | clone | fetch | push  | create a ref | rewinds a ref | delete a ref |
|-------+--------+-------+-------+-------+--------------+---------------+--------------|
| -     | N      | N     | N     | N     | N            | N             | N            |
|-------+--------+-------+-------+-------+--------------+---------------+--------------|
| R     | @Y     | @Y    | @Y    | N     | N            | N             | N            |
...
| M     |        |       |       |       |              |               |              |
|-------+--------+-------+-------+-------+--------------+---------------+--------------|
备注: {@权限字符}	表示在这一列是yes或no由该权限字符说了算
