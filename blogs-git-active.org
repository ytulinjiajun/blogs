...
2. 以refs/heads/开头的是分支指针头
3. 以refs/remotes/开头的是远程版本库分支在本地的映射
4. 以refs/tags/开头的是里程碑
5. HEAD: 
6. master: .git/refs/heads/master
7. stash
8. tag-name: .git/refs/tags/tag-name 
...
** DONE [#D] Theme:反转提交										 :@Blog:@Git:
	...
	- State "DONE"       from "WAITTING"   [2017-10-20 五 10:46]
1. 用途: 在不具备reset权限的前提下,该命令可以将一个错误提交的前面那个提交回滚至工作区,暂存区,以及最新提交处
2. 命令: git reset commit-ID
...
** WAITTING [#D] Theme:merge									 :@Blog:@Git:
    SCHEDULED:<2017-10-23 一>
1. 合并引发冲突的起源
   1. 根本原因: 不同的提交同时修改了同一个文件的相同区域的内容
   2. 一定不会引发冲突的合并: 
	  1. 用户1和用户和同时向远程服务器clone了一个项目
      2. 用户1和用户2分工明确,用户1负责1.c的开发,用户2负责2.c的开发
	  3. 用户1向服务器push了一次1.c修改的提交, 快进式推送成功
	  4. 紧接着用户2也要push对2.c修改的提交,此时用户2会发生预料之中的非快进式推送错误
	  5. 这很正常,用户2需要将用户1的成果fetch下来,同自己的工作成果合并,再进行提交
	  6. 备注: 这样的工作模式是不可能引发冲突的,因为用户1只对1.c进行操作,用户2只对2.c进行操作
   3. 
...
6. 以develop分支最新提交的第n个父提交为新分支起点,创建新分支并切换到新分支: git checkout -b new-br develop~n
7. 切换到“分离头指针状态”
...
8. 开发
   1. 每个开发者都拥有版本库的全部
   2. 服务器上的版本库的master分支是最终的成果,只有
   3. 服务器上的版本库的develop分支,
   4. 每个成员在接收任务时,
9. 分支的类型:
   1. 主分支: master
	  1. 意义: 新发布的一个正式版本
   ...
   3. 辅助分支: 辅助分支的生命周期是有限的,因此不应当push到远端服务器 
      ...
      3. 紧急修复分支: hotfix branch
		 ...
		 12. 备注: 紧急修复分支同发布分支最大的区别在于,紧急修复分支主要针对master分支修复,而发布分支主要针对develop分支修复,其次,紧急修复分支必须同时向master分支以及develop分支合并,而发布分支向master的合并是可选的
Blog creation time:[2017-10-06 五 14:21]
