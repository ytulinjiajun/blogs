...
3. 基于Gitlite搭建git服务器
   ...
   3. Gitolite安装
	  ...
	  3. 安装步骤: 由gitolite管理员来操作
		 ...
		 7. 指定gitolite的管理员: gitolite setup -pk $HOME/gitolite-admin.pub
			...
			2. 该操作会将 gitolite-admin.pub 添加至~/.ssh/authorized-keys中,这在标准SSH中意味着,该用户可以使用 ssh git@server 免密码直接远程登录服务器git账号,在这里,意味着: 第一,gitolite管理员再也无法使用ssh git@server 登录服务器的git账号取得shell了; 第二,gitolite是唯一一个可以使用git clone git@server gitolite-admin.git将gitolite的配置文件所在的这个版本库克隆到本地,修改,然后push上去使配置生效的人
   ...
   4. Gitolite管理员的那些事: (本地完成)
	  ...
	  2. 新建配置文件: ~/.ssh/config
		 ...
		 #+END_SRC	 
	  3. 更改授权文件
	     1. 授权文件: gitolite-admin/conf/gitolite.conf  
		 2. 语法:
	  4. 新增用户:
		 ...
		 6. 备注: 执行完毕push操作之后,将会发现在服务器的git账户的~/.ssh/authorized_keys中追加了新增用户的公钥,这意味着----(待测试),
